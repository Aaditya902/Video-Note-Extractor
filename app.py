...
def _status_html(progress: PipelineProgress) -> str:
    """Convert a PipelineProgress snapshot into styled HTML. """
    ...
    if question:
        ...
        with st.spinner("Thinking..."):
            answer = qa_answer(
                ...
                chat_history = st.session_state.chat_history[:-1],  # history before this question
            )
...
def main() -> None:
    ...
    # Extract button
    if st.button("EXTRACT NOTES", disabled=not st.session_state.ready,
                 use_container_width=True):
